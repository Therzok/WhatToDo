...
local GeminiConfigDialog = APkg and APkg.tPackage or {}

GeminiConfigDialog.OpenFrames = GeminiConfigDialog.OpenFrames or {}
GeminiConfigDialog.Status = GeminiConfigDialog.Status or {}
GeminiConfigDialog.frame = {}

GeminiConfigDialog.frame.apps = GeminiConfigDialog.frame.apps or {}
...
local function FrameOnClose(widget, event)
    ...
    GeminiConfigDialog.OpenFrames[appName] = nil
    gui:Release(widget)
end
...
local function GroupSelected(widget, event, uniquevalue)
    ...
    GeminiConfigDialog:FeedGroup(user.appName, options, widget, rootframe, feedpath)
...
local function RefreshOnUpdate(this)
    for appName in pairs(this.closing) do
        if GeminiConfigDialog.OpenFrames[appName] then
            GeminiConfigDialog.OpenFrames[appName]:Hide()
        end
        if GeminiConfigDialog.BlizOptions and GeminiConfigDialog.BlizOptions[appName] then
            for key, widget in pairs(GeminiConfigDialog.BlizOptions[appName]) do
    ...
    for appName in pairs(this.apps) do
        if GeminiConfigDialog.OpenFrames[appName] then
            ...
            GeminiConfigDialog:Open(appName, unpack(user.basepath or emptyTbl))
        end
        if GeminiConfigDialog.BlizOptions and GeminiConfigDialog.BlizOptions[appName] then
            for key, widget in pairs(GeminiConfigDialog.BlizOptions[appName]) do
                local user = widget:GetUserDataTable()
                if widget:IsVisible() then
                    GeminiConfigDialog:Open(widget:GetUserData("appName"), widget, unpack(user.basepath or emptyTbl))
                end
            end
        end
...
function GeminiConfigDialog:CloseAll()
    GeminiConfigDialog.frame.closeAll = true
    GeminiConfigDialog.frame:SetScript("OnUpdate", RefreshOnUpdate)
...
-- Internal -- Called by GeminiConfigRegistry
function GeminiConfigDialog:ConfigTableChanged(event, appName)
    GeminiConfigDialog.frame.apps[appName] = true
    GeminiConfigDialog.frame:SetScript("OnUpdate", RefreshOnUpdate)
end
...
function GeminiConfigDialog:SetDefaultSize(appName, width, height)
...
function GeminiConfigDialog:Open(appName, container, ...)
    ...
    if GeminiConfigDialog.frame.closeAll then
        -- close all is set, but thats not good, since we're just opening here, so force it
        GeminiConfigDialog.frame.closeAllOverride[appName] = true
    end
end

GeminiConfigDialog.BlizOptions = GeminiConfigDialog.BlizOptions or {}

local function FeedToBlizPanel(widget, event)
    local path = widget:GetUserData("path")
    GeminiConfigDialog:Open(widget:GetUserData("appName"), widget, unpack(path or emptyTbl))
end
